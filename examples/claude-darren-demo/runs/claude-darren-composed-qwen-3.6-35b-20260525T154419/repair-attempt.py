...
def main():
    ...
    if "actions" in act_data and isinstance(act_data["actions"], list):
        for act in act_data["actions"]:
            if not isinstance(act, dict):
                continue
            if "action_type" not in act or "bioregion" not in act:
                continue
    ...
    for i, br in enumerate(sorted_bioregions):
        ...
        # Add blank line after each bioregion section except the last one
        if i < len(sorted_bioregions) - 1:
            output_lines.append("")
