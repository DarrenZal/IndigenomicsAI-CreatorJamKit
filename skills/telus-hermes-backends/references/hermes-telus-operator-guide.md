...
|---|---|---|
| Qwen `Qwen/Qwen3.6-35B-A3B` | Complex reasoning, coding, decomposition, multi-step tool loops | Reasoning can consume output tokens. Start with `max_tokens: 8192`; raise it for long-form generation. |
...
Low values can break reasoning models because internal reasoning and visible output compete for the same response budget. For Qwen/Hermes work, use at least `8192`; raise it further for long planning, long writing, or complicated tool use.
...
Use `max_tokens: 8192` for normal Gateway/Hermes work. Use lower values only for deliberate smoke tests, and raise it for long-form generation.
