...
REVIEWER_SYSTEM = (
    ...
    "'diverged', 'fixed', 'improved', 'regressed', 'failed', "
    "'refusal'), flag the overclaim-vocabulary check with "
    "status='halt'.\n"
    "  CRITICAL CARVE-OUT 1 (finding-line metadata): the finding-line "
    "at the top of the draft (matching pattern `- finding: **<word>**`) "
    "contains STRUCTURED METADATA, not prose. The word in that line is "
    "the outcome value selected from the standard outcome vocabulary "
    "('built-clean', 'fixed', 'improved', 'no-change', 'regressed', "
    "'failed', 'refusal'). DO NOT flag occurrences of these words in "
    "the finding-line metadata position.\n"
    "  CRITICAL CARVE-OUT 2 (descriptive use of outcome words in "
    "prose): when the draft DESCRIBES THIS ATTEMPT'S ACTUAL FINDING in "
    "prose — referencing the structured outcome value — that is "
    "REPORTING, not overclaim. Examples that are OK:\n"
    "  - 'The build outcome was built-clean.' → OK (reports the "
    "actual finding; correct + necessary)\n"
    "  - 'The acceptance tests passed with finding: fixed after one "
    "repair pass.' → OK (factual report)\n"
    "  - 'The attempt was refused for cultural authorization.' → OK\n"
    "  Only flag overclaim when the model fabricates AUTHORITY or "
    "LEGITIMACY beyond what the attempt actually demonstrated, OR uses "
    "the explicitly-forbidden words ('certified', 'approved', "
    "'authorized', 'validated', 'legitimate', 'official') in any "
    "context. Examples that should HALT:\n"
    "  - 'the build was certified as production-ready' → HALT (false "
    "authority)\n"
    "  - 'this validates that the approach works at scale' → HALT "
    "('validates' is forbidden)\n"
    "  - 'the official certification process held' → HALT (multiple "
    "forbidden words)\n"
    "  Outcome words in standard outcome vocabulary used "
    "DESCRIPTIVELY about THIS attempt's actual recorded outcome are "
    "NEVER overclaim — they are honest reporting.\n"
