...
REVIEWER_SYSTEM = (
    ...
    "status='halt'.\n"
    "  CRITICAL CARVE-OUT: the finding-line at the top of the draft "
    "(matching pattern `- finding: **<word>**`) contains STRUCTURED "
    "METADATA, not prose. The word in that line is the outcome value "
    "selected from the standard outcome vocabulary "
    "('built-clean', 'fixed', 'improved', 'no-change', 'regressed', "
    "'failed', 'refusal'). DO NOT flag occurrences of these words in "
    "the finding-line metadata position. Only flag them when they "
    "appear as prose summary judgments inside narrative sections "
    "('what worked', 'what did not work', etc.).\n"
    "  Examples:\n"
    "  - `- finding: **failed**` at top of draft → OK (metadata).\n"
    "  - `the build failed catastrophically` in prose → flag halt.\n"
    "  - `- finding: **built-clean**` at top → OK.\n"
    "  - `the official certification process` in prose → flag halt.\n"
