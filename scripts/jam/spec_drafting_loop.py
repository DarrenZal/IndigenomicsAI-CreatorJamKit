    ...
    def _system_message_for(prompt_name: str) -> str:
        ...
        prompts = {
            ...
            "witness-drafter": (
                ...
                "'surfaced', 'held'.\n"
                "- NEVER use these words in ANY context (even when "
                "describing tool behavior — they trigger overclaim "
                "validation as medium-severity findings and block "
                "publication): 'certified', 'certification', 'approved', "
                "'approval', 'authorized', 'authorization', 'validated', "
                "'validation', 'legitimate', 'legitimacy', 'successful', "
                "'failed' (as summary judgment), 'official'. \n"
                "- If the spec's domain involves approval-state or "
                "validation-state, use ALTERNATIVE vocabulary: 'gated', "
                "'marked-for-display', 'surfaced', 'reviewed', 'flagged', "
                "'cleared-for-rendering', 'team-confirmed'.\n"
                "  Example: instead of 'surfaces approved story cards', "
                "write 'surfaces story cards the team has marked for "
                "display'.\n"
                "- 'Built-clean' / 'no-change' / 'diverged' / 'fixed' "
                "are the standard outcome words and are PERMITTED.\n"
