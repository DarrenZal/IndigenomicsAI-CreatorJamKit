...
# Freshness windows in days per claim type
FRESHNESS_WINDOWS = {
    "descriptive": 730,
...
def validate_claim(claim: dict, idx: int) -> None:
    required = [
    ...
    for ep_idx, ep in enumerate(claim["evidence_pointers"]):
        ...
        # reviewer may be null or string; no further validation
...
def compute_as_of(claims: list, provided: str | None) -> datetime.date:
    ...
    for claim in claims:
        for ep in claim["evidence_pointers"]:
            ...
            if latest is None or d > latest:
                latest = d
    return latest if latest is not None else datetime.date(2026, 5, 26)

def status_for_claim(claim: dict, as_of: datetime.date) -> str:
    ...
    for ep in eps:
        ...
        # stale when age is greater than or equal to the window
        if age < window_days:
            all_stale = False
            break
    if all_stale:
...
def main() -> None:
    ...
    # Build report
    lines = []
    lines.append(f"CLAIMS COHERENCE REPORT (as_of {as_of_date.isoformat()})")
    ...
    for claim in claims:
        ...
        if status == "ready_for_use":
            ...
            fail_cnt += 1
        lines.append(f"{claim['claim_id']} [{claim['claim_type']}]: {status}")
        if status != "ready_for_use":
    ...
    total = len(claims)
    lines.append(f"SUMMARY: {ready_cnt} ready / {fail_cnt} needing work of {total} total")
