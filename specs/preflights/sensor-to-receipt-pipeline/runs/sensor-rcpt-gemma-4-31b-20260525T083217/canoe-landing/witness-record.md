# Canoe Landing / Witness Record — Sensor Receipts preflight

- packet: `preflight-sensor-to-receipt`
- build model: `gemma-4-31b` (TELUS build-attempt lane)
- date: 2026-05-25
- finding: **built clean**

## What we brought

Turn sensor readings or field observations into reviewable evidence packets with public-safe summaries.

## What we attempted

A build attempt on the frozen build packet via the TELUS build-attempt lane. Build target: single-file CLI.

## What worked

The build attempt produced a tool that passed all 6 acceptance tests.

## What did not work / what broke

Nothing in the build attempt.

## What we learned

The frozen build packet carried enough for a working build via the TELUS lane.

## Boundaries that remain

Marker-only records named in the packet were not given to the build attempt and were not computed on: sensitive-coords.

## Receipt

This record states what happened. It does not establish authority, approval, certification, legitimacy, community consent, or readiness for reuse.
