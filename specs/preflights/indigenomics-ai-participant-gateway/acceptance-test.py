#!/usr/bin/env python3
import json, os, subprocess, sys, tempfile, unittest
HERE = os.path.dirname(os.path.abspath(__file__))
TOOL = os.path.join(HERE, "tool.py")
TIMEOUT = 10

def _write(d, n, o):
    p = os.path.join(d, n)
    with open(p, "w") as f:
        if isinstance(o, str): f.write(o)
        else: json.dump(o, f)
    return p

class PreflightTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls): cls.tmp = tempfile.mkdtemp(prefix="pre-")
    def _run(self, *args):
        return subprocess.run([sys.executable, TOOL] + list(args), capture_output=True, text=True, timeout=TIMEOUT)

    def test_t1_route(self):
        p = _write(self.tmp, "t1_route.json", {'name': 'A', 'email': 'a@x', 'requested_spec': 'x', 'consent_team_visibility': True, 'consent_ai_use': 'public', 'consent_display_tuesday': True, 'flag_protected': False})
        proc = self._run(p)
        self.assertEqual(proc.returncode, 0, "stderr=%r" % proc.stderr)
        self.assertIn('DECISION: ROUTE', proc.stdout)

    def test_t2_refuse(self):
        p = _write(self.tmp, "t2_refuse.json", {'name': '', 'email': '', 'requested_spec': 'x', 'consent_team_visibility': True, 'consent_ai_use': 'public', 'consent_display_tuesday': True, 'flag_protected': False})
        proc = self._run(p)
        self.assertEqual(proc.returncode, 0, "stderr=%r" % proc.stderr)
        self.assertIn('DECISION: REFUSE', proc.stdout)

    def test_t3_hold_protected(self):
        p = _write(self.tmp, "t3_hold_protected.json", {'name': 'A', 'email': 'a', 'requested_spec': 'x', 'consent_team_visibility': True, 'consent_ai_use': 'public', 'consent_display_tuesday': True, 'flag_protected': True})
        proc = self._run(p)
        self.assertEqual(proc.returncode, 0, "stderr=%r" % proc.stderr)
        self.assertIn('DECISION: HOLD', proc.stdout)
        self.assertIn('protected flag', proc.stdout)


    def test_zz_invalid_json(self):
        p = _write(self.tmp, "bad", "{not valid")
        proc = self._run(p)
        self.assertEqual(proc.returncode, 0)
        self.assertTrue(proc.stdout.strip().startswith("error:"))


def _main():
    s = unittest.TestLoader().loadTestsFromModule(sys.modules[__name__])
    r = unittest.TextTestRunner(verbosity=2, stream=sys.stderr).run(s)
    print("M2LITE_SUMMARY " + json.dumps({"tests": r.testsRun, "passed": r.testsRun - len(r.failures) - len(r.errors),
                                          "failures": sorted(t.id() for t,_ in r.failures), "errors": sorted(t.id() for t,_ in r.errors)}))
    return 0 if r.wasSuccessful() else 1

if __name__ == "__main__":
    sys.exit(_main())
