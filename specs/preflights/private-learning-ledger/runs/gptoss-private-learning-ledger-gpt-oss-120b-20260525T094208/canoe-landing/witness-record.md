# Canoe Landing / Witness Record — preflight

- packet: `preflight-private-learning`
- build model: `gpt-oss-120b` (TELUS build-attempt lane)
- date: 2026-05-25
- finding: **built clean**

## What we brought

Aggregate steward-action records into a public-safe learning summary without exposing protected data.

## What we attempted

A build attempt on the frozen build packet via the TELUS build-attempt lane. Build target: single-file CLI.

## What worked

The build attempt produced a tool that passed all 4 acceptance tests.

## What did not work / what broke

Nothing in the build attempt.

## What we learned

The frozen build packet carried enough for a working build via the TELUS lane.

## Boundaries that remain

Marker-only records named in the packet were not given to the build attempt and were not computed on: none.

## Receipt

This record states what happened. It does not establish authority, approval, certification, legitimacy, community consent, or readiness for reuse.
