...
| Witness drafting | [Draft Witness v0](draft-witness-v0/) | implemented + tested + chain-demoed | Stage 6 of the drafting chain — takes a frozen build packet + build outputs and produces a witness-record DRAFT via Prompt 3 (stub or gateway). Renderer falls back to "team to fill" placeholders so output is never null. Closes the offering → packet → build → witness → wall loop. |
| Autonomous orchestration | [Orchestrator v0](orchestrator-v0/) | implemented + smoke-tested | Autonomous spec-execution network: given the spec menu, picks N specs, runs each through the full chain, honors refusal at every step (gatekeeper / model / pre-freeze-sanity / validator), publishes built-clean records + refusal-as-record entries to the wall. Composes all 4 prior meta-specs into one CLI. |
